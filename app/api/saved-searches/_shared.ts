...
import { isAlertCadence, type AlertCadence, type SavedFilters } from "@/lib/api/savedSearches";
...
/**
 * Validates `{ name?, filters?, alertCadence? }`; returns an error string or
 * the clean payload. `alertCadence` is always optional.
 */
export function readPayload(
  body: unknown,
  { requireAll }: { requireAll: boolean },
): { name?: string; filters?: SavedFilters; alertCadence?: AlertCadence } | string {
  if (!body || typeof body !== "object") return "Invalid request.";
  const { name, filters, alertCadence } = body as { name?: unknown; filters?: unknown; alertCadence?: unknown };
  const out: { name?: string; filters?: SavedFilters; alertCadence?: AlertCadence } = {};

  if (alertCadence !== undefined) {
    if (!isAlertCadence(alertCadence)) return "Choose how often to get alerts.";
    out.alertCadence = alertCadence;
  }
